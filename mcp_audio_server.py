...
class AudioProcessingMCP:
    ...
    async def start_server(self):
        ...
        self.logger.info(
            "Starting Audio Processing MCP Server",
            extra={
    ...
    async def process_audio_requests(self, client):
        ...
        while True:
            try:
                ...
                self.logger.error(
                    "Error in audio processing loop", extra={"error": str(e)}
                )
    ...
    async def process_single_request(self, client, request):
        ...
        try:
            ...
            self.logger.info(
                ...
                extra={"request_id": request_id, "operation": payload.get("operation")},
            )

            # Update status to processing
            await self.publish_status(
                client, request_id, "processing", "Processing audio file..."
            )
            ...
            await self.publish_status(
                client, request_id, "completed", "Audio processing completed"
            )
            ...
            self.logger.error(
                "Error processing audio request",
                extra={
                    "request_id": request.get("payload", {}).get(
                        "request_id", "unknown"
                    ),
                    "error": str(e),
                },
    ...
    async def execute_audio_operation(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Create temporary file path for potential use
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
            temp_path = temp_file.name

        try:
            # Resolve audio path from various input formats
    ...
    async def save_audio_data(self, audio_data: Any, temp_path: str) -> str:
        """Save audio data to disk and return its path.

        The input can be an existing file path, a base64-encoded string or raw bytes.
        """
        # Existing file path
    ...
    async def process_operations_handler(
    ...
    async def fade_in_audio(
        ...
        return {"output_path": output_path, "fade_duration": fade_duration}
    ...
    async def add_reverb(
        ...
        return {"output_path": output_path, "room_size": room_size, "damping": damping}
    ...
    async def compress_audio(
        ...
        gain_reduction = np.where(
            y_db > threshold, (threshold - y_db) * (1 - 1 / ratio), 0
        )
        ...
        gain_reduction = signal.lfilter(
            [1], [1, -np.exp(-1 / release_samples)], gain_reduction
        )
        gain_reduction = signal.lfilter(
            [1], [1, -np.exp(-1 / attack_samples)], gain_reduction
        )
